...
class BackgroundManager {
  ...
  setupListeners() {
    chrome.storage.local.get(['searchPrefix', 'isAdBlockerEnabled']).then(data => {
      console.log('스토리지에서 설정 가져옴:', data);
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
      if (request.action === 'updateState') {
        ...
        this.currentState.isAdBlockerEnabled = request.state;
        chrome.storage.local.set({ isAdBlockerEnabled: this.currentState.isAdBlockerEnabled });
        sendResponse({status: "ok"});
      }
      return true;
    });
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
      if (area === 'local') {
        if (changes.searchPrefix) {
          console.log('접두어 변경 감지:', changes.searchPrefix);
          this.currentPrefix = changes.searchPrefix.newValue;
          this.updateContextMenu();
        }
        
        if (changes.isAdBlockerEnabled) {
          console.log('광고차단 상태 변경 감지:', changes.isAdBlockerEnabled);
          this.currentState.isAdBlockerEnabled = changes.isAdBlockerEnabled.newValue;
        }
      }
    });
  }
  ...
  // 디시인사이드 URL 리다이렉트 설정
  async setupDCInsideRedirect() {
    console.log('디시인사이드 declarativeNetRequest 설정 시작');
    
    // 초기 설정 확인 및 규칙 적용
    await this.updateDCRedirectRules();
    
    // 설정 변경 감지
    chrome.storage.onChanged.addListener(async (changes, area) => {
      if (area === 'local' && changes.dcRedirectEnabled) {
        console.log('디시인사이드 리다이렉트 설정 변경:', changes.dcRedirectEnabled.newValue);
        await this.updateDCRedirectRules();
      }
    });
  }

  async updateDCRedirectRules() {
    try {
      // 현재 설정 확인
      const data = await chrome.storage.local.get('dcRedirectEnabled');
      const isEnabled = data.dcRedirectEnabled !== false; // 기본값 true
      
      console.log('디시인사이드 리다이렉트 규칙 업데이트, 활성화:', isEnabled);
      
      if (isEnabled) {
        // 규칙 활성화
        await chrome.declarativeNetRequest.updateEnabledRulesets({
          enableRulesetIds: ['dcInside_redirect']
        });
        console.log('디시인사이드 리다이렉트 규칙 활성화됨');
      } else {
        // 규칙 비활성화
        await chrome.declarativeNetRequest.updateEnabledRulesets({
          disableRulesetIds: ['dcInside_redirect']
        });
        console.log('디시인사이드 리다이렉트 규칙 비활성화됨');
      }
    } catch (error) {
      console.error('디시인사이드 리다이렉트 규칙 업데이트 오류:', error);
    }
  }
}
