...
class PopupManager {
  constructor() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      this.dcRedirectToggle = document.getElementById('dcRedirectToggle');
      
      if (!this.toggleSwitch || !this.searchPrefix || !this.saveButton || 
          !this.skipTimeInput || !this.skipTimeSaveButton || !this.adBlockerToggle || !this.dcRedirectToggle) {
  ...
  initializeUI(data) {
    try {
      ...
      if (this.dcRedirectToggle) {
          this.dcRedirectToggle.checked = data.dcRedirectEnabled !== false; // 기본값 true
          this.isDCRedirectEnabled = data.dcRedirectEnabled !== false;
      }
  ...
  setupEventListeners() {
    ...
    if (this.dcRedirectToggle) {
        this.dcRedirectToggle.addEventListener('change', this.handleDCRedirectToggle.bind(this));
    }
  }
  ...
  async handleDCRedirectToggle(e) {
    try {
      const isEnabled = e.target.checked;
      await chrome.storage.local.set({ dcRedirectEnabled: isEnabled });
      this.isDCRedirectEnabled = isEnabled;
      
      // 현재 활성화된 탭에 변경된 상태를 알림
      const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
      if (tab && tab.url && tab.url.includes('dcinside.com')) {
        chrome.tabs.sendMessage(tab.id, {
          action: 'toggleDCRedirect',
          enabled: isEnabled
        }).catch(err => {
          console.log('디시인사이드 리다이렉트 토글 메시지 전송 실패:', err);
        });
      }
      
      setTimeout(() => window.close(), 300);
    } catch (error) {
      console.error('디시인사이드 리다이렉트 상태 변경 오류:', error);
      e.target.checked = !e.target.checked;
    }
  }
